...
const certificateData = [
  {
    src: Visteon,
    title: 'Internship Completion (Visteon)',
    link: 'https://drive.google.com/file/d/15MQwnfu2PSDnOCgi1OyyOKasv7QufvR-/view?usp=sharing',
    label: 'View',
  },
  {
    src: IBMAI2,
  ...
  {
    src: IBMAI,
    title: 'Getting Started with AI (IBM)',
    link: 'https://www.credly.com/go/8EdWLh4f',
    label: 'Verify',
  },
  {
    src: TCS,
    title: 'Young Professional (TCS iON)',
    link: 'https://drive.google.com/file/d/1w4VObVKVfSlycDelJuDwBWjY5RI6BzNF/view',
    label: 'View',
  },
  {
    src: ReactJS,
    title: 'React JS (Udemy)',
    link: 'https://www.udemy.com/certificate/UC-d2e99ccb-4ea2-42b4-b44c-a232bf3412bc/',
    label: 'Verify',
  },
  {
    src: PS,
    title: 'Problem Solving (HackerRank)',
    link: 'https://www.hackerrank.com/certificates/770a3c68cecb',
    label: 'Verify',
  },
];
...
const Certificates = () => {
  return (
    <div
      name='certificates'
      className='relative px-6 w-full min-h-screen text-gray-300 overflow-hidden bg-black bg-opacity-60'
    >
      <div className='max-w-[1000px] mx-auto flex flex-col justify-center w-full h-full z-10'>
        <div className='z-10'>
          <p className='text-4xl font-bold inline border-b-4 border-pink-600'>Certificates</p>
          <p className='py-4'>These are some of the certificates I've received:</p>
        </div>
        ...
        <div className='w-full grid grid-cols-2 sm:grid-cols-3 gap-4 text-center py-8 z-10'>
  {certificateData.map(({ src, title, link, label }, index) => (
    <div
      key={index}
      style={{ backgroundImage: `url(${src})` }}
      className='group relative w-full sm:aspect-video aspect-square bg-cover bg-center shadow-md shadow-[#040c16] rounded-md overflow-hidden'
    >
      <div className='absolute inset-0 opacity-0 group-hover:opacity-100 duration-300 bg-black bg-opacity-70 flex flex-col justify-center items-center px-2'>
        <span className='text-sm sm:text-base font-bold tracking-wide text-white text-center'>
          {title}
        </span>
        <a href={link} target='_blank' rel='noreferrer'>
          <button className='text-gray-700 bg-white rounded-md px-4 py-1 mt-3 font-bold hover:scale-95 duration-300 text-sm'>
            {label}
          </button>
        </a>
      </div>
